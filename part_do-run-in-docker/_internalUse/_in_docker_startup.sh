...
function processActions()
{
    ...
    [[ -n "$msgSuffix" ]] && msgSuffix=" ${msgSuffix}"
    for file in "${EXE_DIR%/}/${prefix}"*; do
        if [ -x "$file" ]; then
            echo "Processing: $file"
            "$file" || echo "⚠️  Failed to execute $file, but continuing${msgSuffix}."
        fi
    done
}
...
function main()
{
    ...
    local hasCmdToRun='no'
    if ((${#option_local_cmd_to_run[@]} != 0)) ; then
        hasCmdToRun='yes'
        echo "Running within docker image [${NAME:-} ${VERSION:-}] ($option_exit) cmd: ${option_local_cmd_to_run[*]}" >&2
    elif [[ "$option_exit" == "--exit=yes" ]] ; then
        echo "Running docker image [${NAME:-} ${VERSION:-}]} ${option_exit}" >&2
    else
        option_exit="--exit=no"
        echo "Running docker image [${NAME:-} ${VERSION:-}]}" >&2
    fi
    ...
    processActions "pre-" "with the rest of the startup script"
    ...
    if [[ "$hasCmdToRun" == "no" ]] ; then

        dumpVersions  | withLeftBox

        if [[ "$option_exit" != "--exit=yes" ]] ; then
            echo "═══════════════════════════════════════════════════════════════════════════"
            echo " In docker image [$option_exit]"
        ...
        processActions "post-" "with the rest of the script"
    ...
    processActions "on-exit-" "with the rest of the exit process."
